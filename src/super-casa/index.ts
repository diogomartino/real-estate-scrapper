...
class SuperCasa extends BaseScrapper implements IScrapper {
  constructor() {
    super();
    this.id = "super-casa";
  }
  ...
  public getPagination = async (url: string): Promise<TPaginationResult> => {
    const html = await scrapper.getRenderedHtml(url, {
      waitUntil: "load",
      waitForSelector: ".property-list-title a, .list-pagination",
    ...
    $(".property-list-title a").each((_, element) => {
  ...
  public buildUrl = ({
    ...
    const url = new URL(
      `https://supercasa.pt/comprar-casas/${city}/com-${types},preco-min-${minPrice},preco-max-${maxPrice},garagem,novo,bom-estado,ultimo-andar,andares-intermedios?ordem=atualizado-desc`,
    );
